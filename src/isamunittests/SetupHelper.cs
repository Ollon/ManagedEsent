...
namespace IsamUnitTests
{
    ...
    internal static class SetupHelper
    {
        ...
        {
            string myDir = IsamTestHelper.PathGetRandomFileName() + @"\";
            IsamTestHelper.DirectoryCreateDirectory(myDir);
            return myDir;
        }
        ...
        public static IsamInstance CreateNewInstance(string myDir)
        {
            // Create instance and set paths.
            IsamInstance instance = new IsamInstance(myDir, myDir, myDir, "edb", "Isam");

            // Small logfiles, no events and small temp db
            instance.IsamSystemParameters.LogFileSize = 256; // 256Kb
            instance.IsamSystemParameters.PageTempDBMin = SystemParameters.PageTempDBSmallest;
            instance.IsamSystemParameters.NoInformationEvent = true;
            return instance;
        }
        ...
        public static void CheckProcessForInstanceLeaks()
        {
            ...
            if (numInstances != 0)
            {
                IsamTestHelper.ConsoleWriteLine("There are {0} instances remaining! They are:", numInstances);
                foreach (var instanceInfo in instances)
                {
                    ...
                    IsamTestHelper.ConsoleWriteLine(
